...
BEGIN
    ...
    FOR _rows IN EXECUTE _missing_query
        LOOP
            _missing_count := _missing_count + 1;
        END LOOP;

    FOR _rows IN EXECUTE _excess_query
        LOOP
    ...
    ELSE
        RETURN _exercise_nr || ' geeft niet de juiste resultaten: er zijn ' || _excess_count::text ||
               ' verkeerde rijen teveel en ' || _missing_count::text || ' goede rijen ontbreken.';
    END IF;
EXCEPTION
...
CREATE OR REPLACE FUNCTION test_exists(_exercise_nr TEXT, _row_test BIGINT,
                                       _compare_type TEXT DEFAULT 'exact') RETURNS TEXT AS
$$
DECLARE
    _sol_name          TEXT := REPLACE(LOWER(_exercise_nr), '.', '_');
    DECLARE _test_name TEXT := _sol_name || '_test';
...
BEGIN
    ...
    RETURN _exercise_nr || ' heeft niet de juiste uitwerking op de database: er moeten ' || _compare_type || ' ' ||
           _row_test || ' rijen of kolommen zijn, maar de database heeft er ' || _row_count || '.';
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE VIEW s1_1_test AS
SELECT column_name
...
CREATE OR REPLACE VIEW s1_2_test AS
SELECT m.naam
FROM medewerkers m
...
CREATE OR REPLACE VIEW s1_4_test AS
SELECT column_name
FROM information_schema.columns
WHERE table_schema = 'public'
...
CREATE OR REPLACE VIEW s1_5_test AS
SELECT naam
FROM medewerkers
WHERE mnr < 9000;
...
SELECT 'S2.1 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s2_2;
CREATE OR REPLACE VIEW s2_2 AS
SELECT 'S2.2 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s2_3;
CREATE OR REPLACE VIEW s2_3 AS
SELECT 'S2.3 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s2_4;
CREATE OR REPLACE VIEW s2_4 AS
SELECT 'S2.4 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;


DROP VIEW IF EXISTS s2_1_test;
CREATE OR REPLACE VIEW s2_1_test AS
...
CREATE OR REPLACE VIEW s2_3_test AS
SELECT *
...
CREATE OR REPLACE VIEW s2_4_test AS
SELECT naam, voorl
FROM medewerkers
WHERE mnr != 7900;

DROP VIEW IF EXISTS s2_5_test;
CREATE OR REPLACE VIEW s2_5_test AS
SELECT cursus
FROM uitvoeringen
...
CREATE OR REPLACE VIEW s2_6_test AS
SELECT naam
FROM medewerkers
WHERE mnr >= 8000
...
CREATE OR REPLACE VIEW s2_7_test AS
SELECT snr
FROM schalen;
...
SELECT 'S3.1 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s3_2;
CREATE OR REPLACE VIEW s3_2 AS
SELECT 'S3.2 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s3_3;
CREATE OR REPLACE VIEW s3_3 AS
SELECT 'S3.3 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s3_4;
CREATE OR REPLACE VIEW s3_4 AS
SELECT 'S3.4 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s3_5;
CREATE OR REPLACE VIEW s3_5 AS
SELECT 'S3.5 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s3_6;
CREATE OR REPLACE VIEW s3_6 AS
SELECT 'S3.6 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;


DROP VIEW IF EXISTS s3_1_test;
CREATE OR REPLACE VIEW s3_1_test AS
SELECT *
...
CREATE OR REPLACE VIEW s3_2_test AS
SELECT *
...
CREATE OR REPLACE VIEW s3_3_test AS
SELECT *
...
CREATE OR REPLACE VIEW s3_4_test AS
SELECT *
...
CREATE OR REPLACE VIEW s3_5_test AS
SELECT naam
FROM medewerkers
WHERE mnr IN (7499, 7934, 7698, 7876);

DROP VIEW IF EXISTS s3_6_test;
CREATE OR REPLACE VIEW s3_6_test AS
SELECT *
...
SELECT 'S4.1 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s4_2;
CREATE OR REPLACE VIEW s4_2 AS
SELECT 'S4.2 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s4_3;
CREATE OR REPLACE VIEW s4_3 AS
SELECT 'S4.3 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s4_4;
CREATE OR REPLACE VIEW s4_4 AS
SELECT 'S4.4 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s4_5;
CREATE OR REPLACE VIEW s4_5 AS
SELECT 'S4.5 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s4_6;
CREATE OR REPLACE VIEW s4_6 AS
SELECT 'S4.6 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s4_7;
CREATE OR REPLACE VIEW s4_7 AS
SELECT 'S4.7 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;


DROP VIEW IF EXISTS s4_1_test;
CREATE OR REPLACE VIEW s4_1_test AS
SELECT *
...
CREATE OR REPLACE VIEW s4_2_test AS
SELECT *
...
CREATE OR REPLACE VIEW s4_3_test AS
SELECT *
...
CREATE OR REPLACE VIEW s4_4_test AS
SELECT *
...
CREATE OR REPLACE VIEW s4_5_test AS
SELECT *
...
CREATE OR REPLACE VIEW s4_7_test AS
SELECT *
FROM (VALUES (14::BIGINT, 150::NUMERIC, 525::NUMERIC)
...
SELECT 'S5.1 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s5_2;
CREATE OR REPLACE VIEW s5_2 AS
SELECT 'S5.2 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s5_3;
CREATE OR REPLACE VIEW s5_3 AS
SELECT 'S5.3 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s5_4a;
CREATE OR REPLACE VIEW s5_4a AS
SELECT 'S5.4a heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s5_4b;
CREATE OR REPLACE VIEW s5_4b AS
SELECT 'S5.4b heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s5_5;
CREATE OR REPLACE VIEW s5_5 AS
SELECT 'S5.5 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s5_6;
CREATE OR REPLACE VIEW s5_6 AS
SELECT 'S5.6 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s5_7;
CREATE OR REPLACE VIEW s5_7 AS
SELECT 'S5.7 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s5_8;
CREATE OR REPLACE VIEW s5_8 AS
SELECT 'S5.8 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;


DROP VIEW IF EXISTS s5_1_test;
CREATE OR REPLACE VIEW s5_1_test AS
SELECT *
FROM (VALUES (7499::NUMERIC(4))
     ) answer (cursist);

DROP VIEW IF EXISTS s5_2_test;
CREATE OR REPLACE VIEW s5_2_test AS
SELECT *
...
CREATE OR REPLACE VIEW s5_3_test AS
SELECT *
...
CREATE OR REPLACE VIEW s5_4a_test AS
SELECT *
...
CREATE OR REPLACE VIEW s5_4b_test AS
SELECT *
...
CREATE OR REPLACE VIEW s5_5_test AS
SELECT *
...
CREATE OR REPLACE VIEW s5_6_test AS
SELECT *
...
CREATE OR REPLACE VIEW s5_7_test AS
SELECT *
FROM (VALUES ('N'::VARCHAR(5), 'SMIT'::VARCHAR(12))
     ) answer (cursist);

DROP VIEW IF EXISTS s5_8_test;
CREATE OR REPLACE VIEW s5_8_test AS
SELECT *
...
DROP VIEW IF EXISTS s9_1;
CREATE OR REPLACE VIEW s9_1 AS
SELECT 'S9.1 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s9_3;
CREATE OR REPLACE VIEW s9_3 AS
SELECT 'S9.3 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s9_4;
CREATE OR REPLACE VIEW s9_4 AS
SELECT 'S9.4 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
DROP VIEW IF EXISTS s9_5;
CREATE OR REPLACE VIEW s9_5 AS
SELECT 'S9.5 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;


DROP VIEW IF EXISTS s9_1_test;
CREATE OR REPLACE VIEW s9_1_test AS
SELECT m.mnr
FROM medewerkers m,
     afdelingen a,
     historie h
WHERE m.afd = 40
  ...
  AND m.maandsal = h.maandsal;

DROP VIEW IF EXISTS s9_3_test;
CREATE OR REPLACE VIEW s9_3_test AS
SELECT *
FROM (VALUES ('Overgang naar opleidingen, met salaris"correctie" 150'::VARCHAR(60), 7369::NUMERIC(4)),
...
DROP VIEW IF EXISTS s9_4_test;
CREATE OR REPLACE VIEW s9_4_test AS
SELECT *
FROM (VALUES ('CLERCKX'::VARCHAR(12), '2008-07-01'::DATE, 'HOOFDKANTOOR'::VARCHAR(20), 4::NUMERIC(2)),
...
DROP VIEW IF EXISTS s9_5_test;
CREATE OR REPLACE VIEW s9_5_test AS
SELECT *
FROM (VALUES ('SMIT', 0::DOUBLE PRECISION),
             ('ALDERS', 11),
             ('DE WAARD', 13),
             ('JANSEN', 17),
             ('MARTENS', 0),
