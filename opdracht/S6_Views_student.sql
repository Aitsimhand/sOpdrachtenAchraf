...
CREATE OR REPLACE VIEW deelnemers AS
    SELECT ins.cursist, ins.cursus, ins.begindatum, uit.docent, uit.locatie FROM inschrijvingen ins, uitvoeringen uit;
...
SELECT DISTINCT * FROM deelnemers JOIN   personeel ON deelnemers.cursist = personeel.mnr;
SELECT DISTINCT * FROM personeel JOIN deelnemers ON personeel.mnr = deelnemers.cursist;
...
CREATE OR REPLACE VIEW dagcursussen AS SELECT cur.code, cur.omschrijving, cur.type FROM cursussen cur WHERE cur.lengte = 1;
